...
def changeOptionDialog(gid: str) -> str:
    ...
    cmd = get_editor_command(temp_file, r"-i NONE -c 'set commentstring=#\ %s'")
    ...
    keys_with_diff_values = set(
        ...
        and current_options[key] != loaded_options.get(key, None)
    )
...
def changeOptionBatchDialog(gids: list) -> str:
    ...
    cmd = get_editor_command(temp_file, r"-c 'set commentstring=#\ %s' -i NONE")
    subprocess.run(cmd, shell=True)
...
def changeOptionPicker(stdscr: curses.window, gid: str) -> str:
    ...
    keys_with_diff_values = set(
        key
        for key in current_options
        if current_options[key] != loaded_options.get(key, None)
    )
...
def filterGlobalOptions(options):
    ...
    acceptable_options = (
        set(input_file_options).difference(set(exclude_input_options))
    ).union(set(global_options))

    options = {
        key: value for key, value in options.items() if key in acceptable_options
    }
...
def changeFilenamePicker(stdscr: curses.window, gid: str) -> str:
    ...
    keys_with_diff_values = set(
        key
        for key in current_options
        if current_options[key] != loaded_options.get(key, None) and key == "out"
    )
...
def download_selected_files(stdscr, gids):
    ...
    for gid in gids:
        ...
        selected_indices = [
            i for i in range(len(files_dict)) if files_dict[i]["selected"] == "true"
        ]
        ...
        if selected_indices != modified_selections and function_data["last_key"] != ord(
            "q"
        ):
