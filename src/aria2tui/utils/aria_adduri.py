...
def addDownloadFull(
    ...
    if cookies_file:
        ...
        with open(cookies_file, "r") as file:
            download_options_dict["header"] = [
                f"Cookie: {line.strip()}" for line in file
            ]
    ...
    response = requests.post(
        url, headers={"Content-Type": "application/json"}, data=json.dumps(request_data)
    )
...
def kitty_prompt(name: str, url: str) -> Tuple[dict, str, list[str]]:
    """Open an editor buffer and return the lines of the saved buffer."""
    ...
    # Open the temporary file in editor within a new kitty window
    editor_cmd = get_editor_command(tmpfile_path, "-i NONE")
    cmd = f"kitty --class=reader-class {editor_cmd}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Add a download to a running aria2c server"
    )
    parser.add_argument("uri", help="The URI of the file to download")
    parser.add_argument(
        "-o", "--output", help="Optional filename for the downloaded file"
    )
    parser.add_argument(
        "-p", "--proxy", help="Optional filename for the downloaded file"
    )
    parser.add_argument(
        "--kitty-prompt", action="store_true", help="Confirm with kitty prompt."
    )
